...
alias rmake='catkin_make -j4 -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_EXPORT_COMPILE_COMMANDS=ON && notify-send "build finished" || notify-send "build failed"'
...
alias psf='ps -o pid,comm,args -u ${UID} --no-headers | fzf'
alias attach='gdb -p $(psf | awk -e "{print $1}")'
alias -g G='|grep --color=auto'
alias -g L='|lv -c'
alias -g F='|fzf'
alias -g C='|column'
alias -g N=';notify-send done "${LASTCMD}"'
...
[ -e $zsh_hightlight_path ] && source $zsh_highlight_path || echo "Failed to load zsh-syntax-highliting."
